...
function handleBombDrop(bombElement, clientX, clientY) {
    ...
    let wrongBombUsed = false; // Flag to ensure score is only reduced once for wrong bomb
    ...
    threats.forEach((threat) => {
        ...
        if (
            ...
            } else if (!scoreUpdated && !wrongBombUsed) {
                // If it's the first wrong bomb drop, penalize score
                wrongBombUsed = true; // Flag it as used
                updateScore(-5); // Decrease score by -5 for a wrong bomb
